...
func (s Shelf) CheckStock(ctx context.Context, item offer.Offer) (offer.StockReading, error) {
	list := s.listOf(item)
	if list == nil {
		return offer.ReadStock("unknown"), ErrNoList
	}
	return list.CheckStock(ctx, item)
}

// listOf Finds the List an Offer came from by the Id that List Wrote on it.
func (s Shelf) listOf(item offer.Offer) *Client {
...
// Pay it again.
func (c *Client) reloadInventory(ctx context.Context, id string) ([]offer.Offer, error) {
	c.mutex.Lock()
