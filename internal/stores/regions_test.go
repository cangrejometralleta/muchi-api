...
func TestResolveAddressReadsStoreRegionNames(t *testing.T) {
	for region, want := range map[string]string{
		...
		got := resolveAddress(model.ShippingAddress{Country: "Chile", Region: region})
		if got.Country != "CL" || got.Region != want {
			t.Errorf("%s: got %+v, want %s", region, got, want)
		}
	}
	if got := resolveAddress(model.ShippingAddress{Country: "AR", Region: "Córdoba"}); got.Region != "Córdoba" {
